...
def _pick_best_artifacts_for_item(item_text: str, artifacts_with_snippets, top_n: int = 2):
    ...
    return [aid for score, aid in scored if score > 0][:top_n]
...
def _safe_get_artifact_ids(control_id: int, k_artifacts: int) -> list[int]:
    """
    IMPORTANT CHANGE:
    - Default to keyword retrieval on Cloud Run (reliable).
    - Only use hybrid (embeddings) if ENABLE_HYBRID_RETRIEVAL=1.
    """
    use_hybrid = os.getenv("ENABLE_HYBRID_RETRIEVAL", "0") == "1"
    ...
    # Always fall back to keyword retrieval (works without HF model)
    try:
        return keyword_retrieve(control_id, k=k_artifacts)
    except Exception as e:
...
def generate_control_report(control_id: int, k_artifacts: int = 5, snippets_per_artifact: int = 2):
    ...
    Never raises, so your route won't return 500.
    """
